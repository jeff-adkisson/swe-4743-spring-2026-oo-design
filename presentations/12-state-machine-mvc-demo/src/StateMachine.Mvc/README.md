...
The application follows the MVC architecture described in [Lecture 11](../../11-mvc-di-srv-domain.md):

- **Controllers** handle HTTP concerns (routing, request binding, response selection) and delegate business logic to an injected service.
- **Service layer** (`IEmailFinderService` / `EmailFinderService`) orchestrates the use case. Controllers depend on the `IEmailFinderService` abstraction, not the concrete implementation.
- **Domain logic** (the `Email/` folder) contains the state machine. It has no knowledge of HTTP, MVC, or the service layer.
- **Composition root** (`Program.cs`) wires the abstraction to the implementation: `builder.Services.AddScoped<IEmailFinderService, EmailFinderService>();`

# Calling the State Machine

Controllers inject `IEmailFinderService` and call it:
...
The service wraps the domain entry point:
