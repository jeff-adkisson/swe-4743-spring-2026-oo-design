...
namespace Demo.Entities
{
    ...
    public class PaperbackBook : IPublication
    {
        ...
        private double _percentageRead; // 0..1
        ...
        private int PagesRemaining
        {
            get
            {
                double remaining = _totalPages * (1.0 - _percentageRead);
        ...
        public void AddPercentageRead(double percentageRead)
        {
            ...
            _percentageRead = Math.Min(1.0, _percentageRead + percentageRead);
        }

        public string GetDetails()
        {
            int percent = (int)Math.Round(_percentageRead * 100);
    ...
    public class Scroll : IPublication
    {
        ...
        private int RemainingMillimeters
        {
            get
            {
                double remaining = _lengthMillimeters * (1.0 - _percentageRead);
                return (int)Math.Ceiling(remaining);
            }
        }
        ...
        {
            int percent = (int)Math.Round(_percentageRead * 100);
            return $"{Title} | Scroll | {_lengthMillimeters} mm total | {percent}% read | {RemainingMillimeters} mm remaining | ~{EstimateReadingMinutes()} min remaining";
        }

        public int EstimateReadingMinutes()
        {
            double minutes = (double)RemainingMillimeters / MillimetersPerMinute;
    ...
    public class AudioBook : IPublication
    {
        ...
        private double _percentageRead; // 0..1 (listened)
        ...
        private int RemainingMinutes
        {
            get
            {
                double remaining = _durationMinutes * (1.0 - _percentageRead);
                return (int)Math.Ceiling(remaining);
            }
        }
